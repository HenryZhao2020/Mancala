...
void SettingsDialog::applySettings() {
    ...
    game->getBoard()->setPlayAI(Attr::get().playAI);
...
StatsDialog::StatsDialog(Game *game)
    ...
    formLayout->setHorizontalSpacing(180);
    formLayout->setVerticalSpacing(10);
    formLayout->setContentsMargins(0, 0, 0, 0);

    // Display red's score
    addRow(Attr::get().playAI ? tr("You") : tr("Player 1"),
           Attr::get().redWins);

    // Display the number of ties between red and blue
    addRow(tr("Tie"), Attr::get().ties);

    // Display blue's score
    addRow(Attr::get().playAI ? tr("AI") : tr("Player 2"),
           Attr::get().blueWins);
