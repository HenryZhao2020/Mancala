...
void Board::moveRed(int i) {
    ...
    QTimer::singleShot(duration + 500, this, [this] {
        ...
        setInteractive(true);
        gameBar->setHintText(Attr::get().redTurn ? "Red's turn again"
                                                 : "Blue's turn");
    });
...
void Board::moveBlue(int i) {
    ...
    QTimer::singleShot(duration + 500, this, [this] {
        // Make the board interactive again
        setInteractive(true);
        gameBar->setHintText(Attr::get().redTurn ? "Red's turn"
                                                 : "Blue's turn again");
    });
...
int Board::getNextBlue() {
    ...
    for (int i = 8; i <= 13; ++i) {
        ...
        if (end >= 8 && end <= 13 && Attr::get().stones[end] == 0 &&
            Attr::get().stones[14 - end] > 0) {
            return i;
        }
...
int Board::getBlueEndIndex(int i) {
    // Starting number of stones
    int stone = Attr::get().stones[i];
...
void Board::redWins() {
    end(Icon::load("Win.svg"), "Red wins!");
...
void Board::blueWins() {
    end(Icon::load(Attr::get().playAI ? "AI.svg" : "Win.svg"), "Blue wins!");
